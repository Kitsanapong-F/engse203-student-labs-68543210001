...
function validateRequest(formData) {
  ...
  if (formData.requesterName.trim().length < 2) {
    errors.requesterName = 'กรุณาระบุชื่องานอย่างน้อย 2 ตัวอักษร';
  }
  ...
  if (formData.location.trim() === '') {
    errors.location = 'กรุณาระบุชื่อสถานที่';
  }

  if (formData.priority === "") {
    errors.priority = 'กรุณาเลือกลำดับความสำคัญ';
  }
  return errors;
}
...
function RequestForm({ onAddRequest }) {
  ...
  const [formData, setFormData] = useState(initialRequests);
  ...
  function handleSubmit(event) {
    ...
    onAddRequest({
      ...formData, requesterName: formData.requesterName.trim(),
    });

    setFormData(initialRequests);
